...
public class ObjectLoader
{
    ...
    public static ArrayList<ArrayList<Float>> IndexObject(ArrayList<ArrayList<Float>> in)
    {
        ...
        for (int i = 0; i < in.get(indexVertices).size(); i++)
        {
            int index = -1;
            {
                ...
                Float[] comparingUV = new Float[]
                        {
                                in.get(indexUvs).get((i * 3) + 0), in.get(indexUvs).get((i * 3) + 1)
                        };
                ...
                for (int j = 0; j < result.get(indexVertices).size(); j++)
                {
                    if
                            (
                                    isNear(comparingVertex[0], result.get(indexVertices).get(i * 3) + 0) &&
                                    isNear(comparingVertex[1], result.get(indexVertices).get(i * 3) + 1) &&
                                    isNear(comparingVertex[2], result.get(indexVertices).get(i * 3) + 2) &&
                                    isNear(comparingUV[0], result.get(indexUvs).get(i * 3) + 0) &&
                                    isNear(comparingUV[1], result.get(indexUvs).get(i * 3) + 1) &&
                                    isNear(comparingNormal[0], result.get(indexNormals).get(i * 3) + 0) &&
                                    isNear(comparingNormal[1], result.get(indexNormals).get(i * 3) + 1) &&
                                    isNear(comparingNormal[2], result.get(indexNormals).get(i * 3) + 2)
                            )
                    {
                        index = i;
                        break;
